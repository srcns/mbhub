...
<p align="center">
  <a href="https://github.com/srcns/mbhub/actions"><img src="https://img.shields.io/badge/tests-157%20passed-brightgreen.svg" alt="Tests"></a>
...
* **L2 — P2P Mesh Swarm (up to 600 ms):** Dispatches encrypted query over libp2p GossipSub to adjacent peers with 50–300 ms anti-correlation jitter.
...
* `Esc`: Close the response viewer and return to a clean query input (answers stay saved in MEMORY).
...
| **Terminal Hijack** | Malicious ANSI escape codes | **Strict State-Machine Sanitizer** strips OSC 52, CSI cursor control, and terminal escapes (`sanitize.rs`). |
...
```
```text
test result: ok. 157 passed; 0 failed; 0 ignored; 0 measured; 0 filtered out; finished in 10.61s
```
