...
fn download_bytes(url: &str, user_agent: &str, timeout: Duration) -> Result<Vec<u8>, String> {
    ...
    if buf.len() as u64 >= MAX_DOWNLOAD_BYTES {
        return Err(format!(
            "Downloaded release file exceeds the {MAX_DOWNLOAD_BYTES}-byte safety cap."
        ));
    }
    if buf.is_empty() {
...
fn resolve_latest_release(user_agent: &str) -> Result<(UpdateSource, String, String), String> {
    ...
    let release: GitHubRelease = match res {
        ...
        Err(ureq::Error::Status(code, _)) => {
            return Err(format!(
                "GitHub API returned HTTP {code}. Please try again later."
            ));
        }
        Err(e) => {
...
fn verify_checksum(asset_name: &str, bytes: &[u8], sums_text: &str) -> Result<(), String> {
    ...
    if actual == expected {
        ...
        Err(
            "SHA-256 checksum mismatch — downloaded file does not match the release manifest"
                .to_string(),
        )
    }
}
...
/// Resolves the latest release version and reports whether it is strictly
/// newer than the running binary. `Err` on network/parse failure — callers
/// treat that as "unknown" and retry later. Cheap: two HTTPS GETs at most.
pub fn check_latest() -> Result<Option<String>, String> {
    let current_version = env!("CARGO_PKG_VERSION");
    let user_agent = format!("mbhub-updater/{current_version}");
    let (_source, latest_tag, latest_version) = resolve_latest_release(&user_agent)?;
    let _ = latest_tag;
...
/// Downloads the newest platform artifact, verifies it against the release's
/// mandatory SHA256SUMS.txt, and atomically replaces the running binary.
/// Performs NO service restart — the CLI path restarts the daemon itself;
/// the daemon path exits so its supervisor restarts it into the new build.
pub fn apply_update() -> Result<(), String> {
    let user_agent = format!("mbhub-updater/{}", env!("CARGO_PKG_VERSION"));
    let current_exe =
        std::env::current_exe().map_err(|e| format!("Cannot locate current executable: {e}"))?;

    let (source, _latest_tag, _latest_version) = resolve_latest_release(&user_agent)?;
    ...
    let (asset_name, asset_url, sums_url) = match &source {
        ...
        UpdateSource::GitHub(release) => {
            let asset = release
                ...
                .ok_or_else(|| {
                    format!("No release binary found for {base_name} in the release.")
                })?;
            let sums = release
                .assets
                .iter()
            ...
            (asset.name.clone(), asset.browser_download_url.clone(), sums)
        }
    };
    ...
    let mut fetch_sums = |url: &str| download_bytes(url, &user_agent, Duration::from_secs(60));
    ...
    if !looks_like_executable(&binary_bytes) {
        return Err(
            "Extracted payload does not look like an executable — aborting update.".to_string(),
        );
    }

    println!(
        "Applying atomic executable update to: {}",
        current_exe.display()
    );
...
pub fn execute_update(check_only: bool) -> Result<(), String> {
    ...
    let Some(latest_version) = check_latest()? else {
...
fn replace_executable_atomically(
    ...
    fs::write(&tmp_path, new_bytes).map_err(|e| {
        format!(
            "Failed to write temporary binary at {}: {e}",
            tmp_path.display()
        )
    })?;
    ...
    #[cfg(unix)]
    {
        fs::rename(&tmp_path, exe_path).map_err(|e| {
            format!("Atomic rename failed: {e}. You may need elevated permissions (sudo).")
        })?;
    }
    ...
    #[cfg(windows)]
    {
        ...
        if let Err(e) = fs::rename(&tmp_path, exe_path) {
            // Rollback (hardening): an AV/EDR lock on the target path must
            // never leave the user with NO binary at the install location.
            let _ = fs::rename(&old_path, exe_path);
            let _ = fs::remove_file(&tmp_path);
            return Err(format!("Failed to replace Windows executable: {e}"));
        }
...
mod tests {
    ...
    fn update_rejected_without_checksum_manifest() {
        ...
        let result =
            verify_release_asset("mbhub-linux-x64.tar.gz", b"bytes", None, |_| Ok(Vec::new()));
        let err = result.unwrap_err();
        assert!(
            err.contains("SHA256SUMS.txt"),
    ...
    fn update_verifies_against_fetched_manifest() {
        ...
        assert!(
            verify_release_asset(
                ...
                &bytes,
                Some("https://example/SHA256SUMS.txt"),
                &mut fetch_ok
            )
            .is_ok()
        );
        ...
        let tampered = b"release-bytes-EVIL".to_vec();
        let mut fetch_ok2 =
            move |_url: &str| -> Result<Vec<u8>, String> { Ok(sums.clone().into_bytes()) };
        assert!(
            verify_release_asset(
                "mbhub-linux-x64.tar.gz",
                &tampered,
    ...
    fn tar_extraction_enforces_decompression_bomb_cap() {
        ...
        let ok =
            extract_binary_capped("mbhub-linux-x64.tar.gz", &archive, 2 * 1024 * 1024).unwrap();
        assert_eq!(ok, payload);

        // The public entry point uses the 256 MiB production cap.
        assert_eq!(
            extract_binary("mbhub-linux-x64.tar.gz", &archive).unwrap(),
            payload
        );
    }
